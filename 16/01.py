...
class SimulationState:
  ...
  def __hash__(self): # Abuse hash out of laziness
    return hash(tuple(self.alreadyVisited))

originState = SimulationState('AA', [], set([valve.label for valve in allValves.values() if valve.rate > 0]), 30, 0)
...
while True:
  ...
  for state in simulationStates:
    ...
    for destinationLabel in state.unvisited:
      ...
      alreadyVisited.append(destinationLabel)
